...
class SkillConsistencyChecker:
    ...
    def scan_all_skills(self):
        ...
        for base_path, location_name in skill_locations:
            ...
            if skills_dir.exists():
                print(f'[DIR] Scanning {location_name} directory...')
        ...
        for skill, deps in dependency_graph.items():
            for dep in deps:
                if dep in dependency_graph and skill in dependency_graph[dep]:
                    dependency_issues.append(f'Circular dependency detected: {skill} <-> {dep}')
    ...
    def generate_consistency_report(self):
        ...
        print('\n' + '='*60)
        print('[SCAN] SKILL CONSISTENCY ANALYSIS REPORT')
        print('='*60)
        ...
        if all_issues:
            print(f'\n[WARN]  Consistency Issues Found ({len(all_issues)}):')

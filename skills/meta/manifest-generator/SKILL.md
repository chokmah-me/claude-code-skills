...
description: Automatically generate and update the unified skill manifest. Use when adding new skills, reorganizing structure, or maintaining skill catalog accuracy.
---

# Manifest Generator Skill

## Purpose
Automatically scan the skills directory structure and generate/update the unified skill manifest that Claude Code can discover and read.

## When to Use
- After adding new skills to any category
- When reorganizing skill structure
- To ensure manifest accuracy with actual skills
...
```bash
# Navigate to skills root
cd ~/.claude/skills

# Find all SKILL.md files (excluding manifest itself)
find . -name "SKILL.md" -not -path "./UNIFIED_SKILL_MANIFEST.md" \
  -not -path "./meta/manifest-generator/*" | sort
```
...
2. **Extract category** from directory path
3. **Identify trigger phrases** from description
4. **Estimate token usage** from skill complexity
5. **Build skill entry** with location and usage info

### Step 3: Generate Categories

Organize skills into categories:

- **development/** - Active coding workflows
- **git/** - Version control operations  
- **analysis/code/** - Code quality analysis
- **analysis/formal/** - Formal verification
- **meta/** - System improvement skills
- **root level** - Legacy/uncategorized skills

### Step 4: Create Manifest Content

Structure the manifest with:

1. **Header** with total count and date
2. **Quick reference** table (most used skills)
3. **Complete catalog** by category
4. **Usage patterns** and workflows
5. **Statistics** and maintenance info

### Step 5: Write Unified Manifest

Generate `UNIFIED_SKILL_MANIFEST.md` with:

```markdown
---
name: unified-skill-manifest
description: Master catalog of all X Claude Code skills organized by category...
---

# 🎯 Claude Code Unified Skill Manifest

**14+ Skills Available** • Organized by Category • Last Updated: YYYY-MM-DD
...
#!/usr/bin/env python3
"""
Claude Code Skills Manifest Generator
Scans skill directories and generates unified manifest
"""
...
```bash
cd ~/.claude/skills
python3 meta/manifest-generator/generate_manifest.py
```

### After Adding New Skills
1. Create new skill in appropriate category
2. Test the skill works
3. Run manifest generator
4. Verify new skill appears in manifest

### Weekly Maintenance
```bash
# Check for skills not in manifest
python3 meta/manifest-generator/generate_manifest.py --check

# Auto-update manifest
python3 meta/manifest-generator/generate_manifest.py --update
```

## Integration with Claude Code

The manifest generator:
1. **Auto-runs** when new skills are detected
2. **Updates** the unified manifest for discovery
3. **Maintains** skill categorization and metadata
4. **Provides** usage statistics and recommendations

## Benefits

- **Discovery**: All skills visible to Claude Code
- **Organization**: Clear categorization system
- **Maintenance**: Automated updates
- **Documentation**: Comprehensive skill catalog
- **Efficiency**: Quick skill identification

---

**💡 Note**: Run this generator whenever you add, modify, or reorganize skills to keep the manifest current!